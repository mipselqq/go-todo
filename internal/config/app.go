...
func NewAppFromEnv(logger *slog.Logger) App {
	...
	jwtExpStr := getEnvStringOrDefault("APP_JWT_EXP", "24h", logger)
	jwtExp, err := time.ParseDuration(jwtExpStr)
	if err != nil {
		jwtExp = 24 * time.Hour
	}

	allowedOrigins := getEnvStringOrDefault("APP_ALLOWED_ORIGINS", "http://localhost:8080,http://127.0.0.1:8080", logger)
	return App{
		Port:           getEnvStringOrDefault("APP_PORT", "8080", logger),
		AdminPort:      getEnvStringOrDefault("APP_ADMIN_PORT", "9091", logger),
		Host:           getEnvStringOrDefault("APP_HOST", "0.0.0.0", logger),
		LogLevel:       getEnvStringOrDefault("APP_LOG_LEVEL", "info", logger),
		Env:            getEnvStringOrDefault("APP_ENV", "dev", logger),
		SwaggerHost:    getEnvStringOrDefault("APP_SWAGGER_HOST", "127.0.0.1:8080", logger),
		JWTSecret:      secrecy.SecretString(getEnvStringOrDefault("APP_JWT_SECRET", "very_secret", logger)),
		JWTExp:         jwtExp,
